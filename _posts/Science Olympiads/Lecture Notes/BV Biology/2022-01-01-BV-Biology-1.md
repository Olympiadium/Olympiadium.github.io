...
이때, 알로스테릭 조절자로 ATP, 시트르산, AMP, ADP 등이 작용한다. 아래의 그래프에는 억제제가 거의 없을 때, 1mM의 ATP가 첨가되었을 때, 1mM의 ATP에 0.1mM의 AMP가 추가로 첨가되었을 때, 각각의 경우에서 PFK-1의 활성 정도를 상대적으로 나타내었다. 아래 그래프를 보면, ATP는 이 효소를 억제하는 음성 조절자로, AMP는 양성 조절자로 작용하였음을 알 수 있다.<br class="tiny">
실제로, ATP와 시트르산은 세포 호흡의 최종, 혹은 중간 산물로 PFK-1의 활성을 억제하는 음성 조절자로 작용하며, AMP나 ADP는 세포 호흡의 최종 산물의 재료가 되는 물질로 PFK-1의 활성을 촉진하는 양성 조절자로 작용한다.